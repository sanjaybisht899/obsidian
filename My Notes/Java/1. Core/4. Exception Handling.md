...
- **Checked Exception**: Requires `throws` in the method declaration and must be handled with `try-catch` or declared further.
- **Unchecked Exception**: No need for `throws` it propagates automatically unless caught.
